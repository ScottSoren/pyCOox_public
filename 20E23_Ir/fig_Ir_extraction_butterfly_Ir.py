...
if True:  # correct H2 signal and ^{16}O^{18}O signal for CO, and ratio
    ...
    extraction.mdict["O2_M34"].cal_mat["M28"] = -2.3e-5 * extraction.mdict["H2"].cal_mat["M2"]
...
if True:  # figure 10b
    ...
    axes = extraction.plot_vs_potential(
        ...
        tspan=tspan_c1, logplot=False, linestyle="-", unit="pmol/s",
        right_space=0.85
    )
    axes = extraction.plot_vs_potential(
        mols=[[H2], [excess_O2, excess_CO2]], t_bg=tspan_bg,
        tspan=tspan_c2, logplot=False, linestyle="--", unit="pmol/s",
        ax=axes)
    axes = extraction.plot_vs_potential(
        mols=[[H2], [excess_O2, excess_CO2]], t_bg=tspan_bg,
        tspan=tspan_c3, logplot=False, linestyle=":", unit="pmol/s",
        ax=axes)
