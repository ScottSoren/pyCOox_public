...
with open('./pickles/20A31_18O_01.pkl', 'rb') as f:
    data = pickle.load(f)
V_str, J_str = sync_metadata(data, RE_vs_RHE=0.715, A_el=0.196#*1e-3, J_str='J / [$\mu$A cm$^{-1}$]'
                             )
t_str = 'time/s'
...
if getgamma:
    ...
    for mass in ['M32', 'M34', 'M36']:
        x, y = mdict['O2_' + mass].get_flux(data, tspan=tspan_O2,
                                            unit='nmol/s', t_bg=t_bg)
        Y[mass] = np.mean(y)
...
plotit = True
if plotit:
    ax2 = plot_experiment(data_burst, mols=[], tspan=[-2, 100],
                          t_bg=t_bg,
                          logplot=False,)
...
for i, mass in [(0,'M44'), (1,'M46'),(2,'M48')]:
    x0, y0 = mdict['CO2_' + mass].get_flux(data_burst, tspan=tspan_CO2,
