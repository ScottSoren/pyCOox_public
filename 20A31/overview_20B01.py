...
if True: # it's a bit slow...
    ...
    ax.legend()
    plt.savefig('./overviews/' + folder + '_MS_data.png') # and save it as an overview plot.
...
EC_files = [f for f in data_list if #re.search(r'\A[0-9]+', f) and 
            '.mpt' in f
...
for tag in tags: # loop over the EC file names.
    ...
    EC_data = load_EC_set(data_folder, tag=tag, 
                          #fix_CP=True 
