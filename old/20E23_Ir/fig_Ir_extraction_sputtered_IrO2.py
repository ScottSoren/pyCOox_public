...
extraction = Extraction.load(
    file, data_dir=data_dir, mdict=mdict
)  # Ir^{18}O2 in H2^{16}O
if True:  # correct H2 signal and ratio
    extraction.mdict["H2"].cal_mat = {
        "M4": -8.5e-4 * 1 / extraction.mdict["H2"].F_cal,
...
if True:  # correct calibration
    ...
    extraction.mdict["O2_M34"].cal_mat["M28"] = (
        -8e-6 * extraction.mdict["O2_M34"].cal_mat["M34"]
    )
...
if True:  # calculate fluxes
    ...
    x_post_CO2, y_post_CO2 = extraction.get_flux(
        CO2, tspan=tspan_post_CO_OER, unit="mol/s/cm^2"
    )
    x_post_O2, y_post_O2 = extraction.get_flux(
        O2, tspan=tspan_post_CO_OER, unit="mol/s/cm^2"
    )
    x_post_x, y_post_x = extraction.get_flux(
        excess_CO2, tspan=tspan_post_CO_OER, unit="mol/s/cm^2"
    )
    x_post_O2_x, y_post_O2_x = extraction.get_flux(
        excess_O2, tspan=tspan_post_CO_OER, unit="mol/s/cm^2"
    )
    ...
    print(
        f"CO2 in long extraction: {n_ex_CO2_all*1e9} nmol/cm^2 out of {n_CO2_all*1e9} nmol/cm^2"
    )
    print(
        f"CO strip is {n_CO_strip*1e9} nmol/cm^2, compared to {n_ML*1e9} nmol/cm^2 surface Ir atoms"
    )

    print(
        f"CO2 after strip: {n_post_CO2_x*1e12} pmol/cm^2 out of {n_post_CO2*1e9} nmol/cm^2"
    )
    print(
        f"O2 after strip: {n_post_O2_x*1e12} pmol/cm^2 out of {n_post_O2*1e9} nmol/cm^2"
    )
