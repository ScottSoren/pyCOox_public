...
H2.cal_mat = {'M2': 1 / H2.F_cal,
              'M4': -0.0007 / H2.F_cal}  # gets rid of the background due to He double-ionization

with open('../pickles/20A31_18O_01.pkl', 'rb') as f:
    data = pickle.load(f)

V_str, J_str = sync_metadata(data, RE_vs_RHE=0.715, A_el=0.196
...
if True:  # plot CO stripping experiment
    ...
    t_bg = [t - tspan_CO_strip[0] for t in
            t_bg_abs]  # it should be relative to the start of the interval.
    ...
    axes1[0].get_figure().set_figwidth(
        figwidth * 1.1)  # to make room for right y-axis label
    ...
    axes2[0].get_figure().set_figwidth(
        figwidth * 0.9)  # to compensate for other panel's right y-axis label
...
if True:  # plot bulk CO oxidation experiment
    ...
    axes1 = plot_experiment(data_ox, mols=[[CO, He],
                                           [H2, CO2_M44, CO2_M46, CO2_M48, O2_M32,
                                            O2_M34, O2_M36]],
                            removebackground='right', t_bg=t_bg, logplot=False,
                            spec={'linewidth': 0.5},
                            unit_left="nmol/s", unit_right="pmol/s/cm^2",)
    ...
    axes2 = plot_vs_potential(data_ox,
                              mols=[CO2_M44, CO2_M46, CO2_M48, H2, O2_M32, O2_M34,
                                    O2_M36],
                              left_space=0.2,  # to avoid cutting the axis label
                              logplot=False, t_bg=t_bg, tspan=tspan_c1,
                              spec={'linewidth': 0.5}, unit="pmol/s/cm^2")
    plot_vs_potential(data_ox,
