...
dataset = Dataset("../pickles/20A31_18O_02.pkl")

V_str, J_str = dataset.sync_metadata(
    RE_vs_RHE=0.715, A_el=0.196  # *1e-3, J_str='J / [$\mu$A cm$^{-2}$]'
)
...
dataset_ox = dataset.cut(tspan=[7400, 10100], t_zero="start")


if True:  # the entire oxidation experiment thing
    dataset_ox.set_background(t_bg=[250, 320])
    ...
    axes[0].get_figure().set_figwidth(
        figwidth * 1.1
    )  # to compensate for other panel's right y-axis label
    axes[0].get_figure().set_figheight(figheight)
    axes[2].set_yticks([-0.4, -0.2, 0, 0.2])
    for ax_i in axes:
        ax_i.set_xlabel("time / [s]")

    plt.savefig("fig_ox_1_Ir.png")
    plt.savefig("fig_ox_1_Ir.svg")


if True:  # Cyclic voltammagram figure
    cv_ox = CyclicVoltammagram(dataset_ox, tspan=[2000, 3000], t_zero=None)

    # cv_ox.reset()
    cv_ox.set_background(t_bg=[2380, 2400])

    cv_ox.redefine_cycle(redox=1, V=0.95)
    ...
    red_2.plot(
        ax=ax, linestyle="--", mols=[CO2_M44, CO2_M46, CO2_M48, H2], logplot=False
    )

    dQ = cv_ox.get_difference(
        cycle_1=0, cycle_2=1, Vspan=[0.6, 0.05], redox=0, ax=ax[1], color="c", alpha=0.5
    )

    ax[0].set_ylabel("cal. signal / [pmol s$^{-1}$]", fontsize=fontsize)
    ax[0].set_ylim([-1, 10.5])
    ax[0].get_figure().set_figwidth(
        figwidth * 0.9
    )  # to compensate for other panel's right y-axis label
    ax[0].get_figure().set_figheight(figheight)
    for ax_i in ax:
        ax_i.set_xlim([0, 1.25])
    ax[1].set_yticks([-0.4, -0.2, 0, 0.2])

    plt.savefig("fig_ox_2_Ir.png")
    plt.savefig("fig_ox_2_Ir.svg")
