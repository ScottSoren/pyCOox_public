...
from EC_MS import (load_calibration_results,
                   Chem, Chip, Molecule,
                   Dataset, CyclicVoltammagram,
                   stagnant_operator, plot_operation,
                   standard_colors
                   )
...
V_str, J_str = dataset.sync_metadata(RE_vs_RHE=0.715, A_el=A_el#*1e-3, J_str='J / [$\mu$A cm$^{-2}$]'
                             )
...
if fitL:
    ...
    D = H2.D # in [m^2/s]
    p0 = 1e5 # in Pa
    L = p0 / kH * D * (2 * Chem.Far * 0.196e-4 / I_HOR ) # [Pa / (Pa / (mol/m^3)) * m^2/s * C/mol * m^2 / (C/s)] = [m]
...
if modelCV:
    ...
    cv.plot_experiment(J_str='cycle',
                       #J_str = 'scan_rate',
                       )
    ...
    if plotit:
        ...
        ax1 = cycle_CO.plot_vs_potential(mols=[H2, CO2_M44, CO2_M46, CO2_M48, O2_M32, O2_M34, O2_M36],
                                logplot=False)
        cycle_He.plot_vs_potential(mols=[H2, CO2_M44, CO2_M46, CO2_M48, O2_M32, O2_M34, O2_M36],
                                       linestyle='--', ax=ax1)
    ...
    if predictsignal:
        ...
        ax3 = diff.plot_experiment(mols=[CO2_M44, CO2_M46, CO2_M48],
                              tspan=tspan_model, plotpotential=False,
                              J_color = 'k', tspan_EC=tspan_EC,
                              logplot=False)
...
if True: # make a 3-panel plot connecting j_diff to the predicted signal
    ...
    for m in [CO2_M44, CO2_M46, CO2_M48]:
        x, y = cycle_CO.get_flux(m, unit='pmol/s',
