...
dataset.sync_metadata(A_el=0.196)
...
ax_0 = dataset.plot_experiment(
       unit = "nA", tspan=[0, 7500], emphasis=None,
)
...
for mass in ['M44', 'M46', 'M48']:
    ...
    m = dataset.point_calibration(mol='CO2', mass=mass, tspan=tspan_CO2,
                          n_el=2)
    m.primary = mass
    F_cal_CO2 += m.F_cal
    m.name = 'CO2_' + mass
...
for mass in ['M32', 'M34', 'M36']:
    m = dataset.point_calibration(mol='O2', mass=mass, tspan=tspan_O2,
                          n_el=4, t_bg=t_bg_O2)
    m.primary = mass
    F_cal_O2 += m.F_cal
    mdict['O2_' + mass] = m
...
H2, ax = dataset.calibration_curve(
        ...
        tspans=[[17250-t_zero, 17300-t_zero],
                [17510-t_zero, 17530-t_zero],
                [17725-t_zero, 17750-t_zero]],
        out=["Molecule", "ax"],
        unit="nA",
)
ax_cal_curve_H2 = ax[1][2]
...
chip = chip_calibration(data_MS, mol=mdict['O2_M32'],
                        tspan=tspan_air, tspan_bg=tspan_He)
chip.save('DanChip-14-20A31')

He = point_calibration(data_MS, mol='He', cal_type='external', chip=chip,
                         tspan=tspan_He, tspan_bg=tspan_air)
mdict['He'] = He

CO = point_calibration(data_MS, mol='CO', cal_type='external', chip=chip,
                         tspan=tspan_CO, tspan_bg=tspan_He)
mdict['CO'] = CO

H2_2 = point_calibration(data_MS, mol='H2', cal_type='external', chip=chip,
                         tspan=tspan_H2, tspan_bg=tspan_air)
# fuck that, for some reason that looks even worse.

save_calibration_results(mdict, '20A31_calibration_results.pkl')


mdict, ax_F_vs_f = recalibrate(internal = [mdict['CO2_M44'], mdict['O2_M32'], mdict['H2']],
            external = [He, CO, H2_2],
            )
